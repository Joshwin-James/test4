  ...
  setTimeout(()=>{
    ...
    try {
      if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
        ...
        // 7. Scroll Indicator
        tl.fromTo("#scrollInd", { opacity: 0, y: -20 }, { opacity: 1, y: 0, duration: 1 }, 2);
...
window.addEventListener('scroll', () => {
  ...
  currentScrollY>500?document.getElementById('btt').classList.add('show'):document.getElementById('btt').classList.remove('show');
});
...
// ── WORKFLOW CYCLE ───────────────────────────────────────────
const wfs=document.querySelectorAll('.wf-step');let wi=0;
setInterval(()=>{wfs.forEach(s=>s.classList.remove('on'));wi=(wi+1)%wfs.length;wfs[wi].classList.add('on')},2000);
...
// ═══════════════════════════════════════════════════════════
        ...
        if(metricCards.length > 0) {
          // Set initial state
          gsap.set('.why-header', { y: 40, autoAlpha: 0 });
          gsap.set('.metric-card', { y: 40, autoAlpha: 0 });
          ...
          ScrollTrigger.create({
            ...
            onEnter: () => {
              // Fade in heading
              gsap.to('.why-header', { y: 0, autoAlpha: 1, duration: 1, ease: "power3.out" });
              
              // Fade in cards
              gsap.to('.metric-card', { y: 0, autoAlpha: 1, duration: 1, stagger: 0.1, delay: 0.3, ease: "power3.out" });
              ...
              metricCards.forEach((el, i) => {
                ...
                gsap.to(obj, {
                  val: target,
                  duration: 2,
                  delay: i * 0.1 + 0.3, // matches card stagger
                  ease: "expo.out",
                  onUpdate: () => {
        ...
        if(indCards.length > 0 && window.innerWidth > 900) {
          ...
          indCards.forEach((card, i) => {
            if(i === 0) {
              gsap.set(card, { zIndex: 10, top: "0%" });
            } else {
              // Hide cards physically below the active one using top percentage instead of transform yPercent
              gsap.set(card, { zIndex: 10 + i, top: "120%" });
            }
          });
          ...
          indCards.forEach((card, i) => {
            ...
            indTl.addLabel("card" + i)
              .to(card, { top: "0%", ease: "power2.inOut" }, "card" + i)
              .to(prevCard, { scale: 0.94, opacity: 0.3, ease: "power2.inOut" }, "card" + i);
          });
        }
